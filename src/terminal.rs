                ...
                match terminal_size() {
                    Ok(new_size) => {
                        if new_size != current_size {
                            info!("terminal resized (from {:?} to {:?})", current_size, new_size);
    ...
    fn poll(&mut self) -> Poll<Option<Self::Item>, Self::Error> {
        loop {
            match self.size.poll() {
                Ok(Async::Ready(Some(size))) => return Ok(Async::Ready(Some(TerminalEvent::Resize(size)))),
                Ok(Async::Ready(None)) => {
        ...
        loop {
            match self.stdin.poll() {
                Ok(Async::Ready(Some(event))) => return Ok(Async::Ready(Some(TerminalEvent::Input(event)))),
                Ok(Async::Ready(None)) => {
