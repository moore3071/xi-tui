...
use errors::*;
use terminal::{TerminalEvent, Terminal};
use view::View;
...
impl Tui {
    ...
    pub fn handle_resize(&mut self, size: (u16, u16)) {
        ...
        let future = self.client
            .scroll(&self.current_view, u64::from(size.0), u64::from(size.1))
            .map_err(|_| ());
    ...
    pub fn click(&mut self, x: u64, y: u64) {
        ...
        if let Some(view) = views.get_mut(current_view) {
            ...
            let future = client
                .click(&self.current_view, line, column)
                .map_err(|_| ());
            handle.spawn(future);
        }
    ...
    pub fn drag(&mut self, line: u64, column: u64) {
        let future = self.client
            .drag(&self.current_view, line, column)
            .map_err(|_| ());
        self.handle.spawn(future);
    }
